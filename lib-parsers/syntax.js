...
var syntax = {
     ...
     */
    highlightCode : function(strCode){
     ...
     * @private
     */
    removeStartWhitespaces : function(strCode){
        var lines = strCode.split(/\n\r?/);
        for (var min = 1000, i = 0, l = lines.length; i < l; i++){
            min = Math.min(min, lines[i].match(/^(\s+)?[^\s]/) && RegExp.$1.length || 1000);
            if (!min) break;
     ...
     * @private
     */
    convertAmlToJson : function(strCode){
     ...
     * @return {String} the highlighted string.
     */
    highlightCode2 : function(strCode){
      var comment=[],str=[];
      return strCode
