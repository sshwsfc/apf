...
var Http = function(){
    ...
    this.toString = this.toString || function(){
        return "[Ajax.org Http Component : (HTTP)]";
    };
    ...
    this.get = this.$get = function(url, options){
        ...
        try {
            ...
            if (!apf.isWebkit)
                setRequestHeader("User-Agent", "Ajax.org Http 3.0"); //@deprecated
    ...
    /**
     * Checks if the request has timed out. If so it's retried
     ...
     *   apf.ERROR    an error has occurred while making the request.
     *   LiveEdit.OFFLINE  the request was made while the application was offline.
     * @param {DOMNode} [amlNode]    the element receiving the error event.
...
//@TODO!!!
        ...
        //fetch code (put up in get)
        if (httpObjectCache.length)
