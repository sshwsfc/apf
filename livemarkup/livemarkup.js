...
//    }
    ...
    function _valed(n, m, x, options, editMode){   // wrap a value with editable div
        ...
        if (editMode !== false) {
            return '<div' 
              + ' onmousedown="LiveEdit.mousedown(this, event)" class="liveEdit' + (options && options.multiline ? ' liveeditMultiline' : '') + (!res && options && options.initial ? ' liveEditInitial' : '') + '" xpath="' + (n 
                ? ((m.substr(0,1) != "/" 
                    ? apf.xmlToXpath(n, null, false) 
                    : "") + "/" + m).replace(/([\[\{\}\]])/g, "\\$1")
                : "") + '"' 
